...
def imf_etl(countries, 
    ...
    try:
        for i in range(min(len(countries), len(counterparts))):
            ...
            file_path = f"/home/ubuntu/result/{df.name}_data.csv"
            df.to_csv(file_path, index=False)
            print(f'SUCCESS: {df.name} data has been created and stored as .csv file')  
    except Exception as e:
        print(e)

def imf_transformation(csv_name_list):
...
imf_etl(countries=countries,
        counterparts=counterparts,
        start_year=2000,
        end_year=2023,
        frequency="M")

imf_transformation(csv_name_list=["W00-W00"])