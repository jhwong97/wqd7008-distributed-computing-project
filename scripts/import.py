...
def mets_transformation(df):
    ...
    df['total_import'] = df.iloc[:,1:].sum(axis=1)
    print(f"SUCCESS: Data Transformation is completed.")
    file_path = "/home/ubuntu/result/import_data_transformed.csv"
...
exports_etl(url, dataframe_name="my_import", file_path="/home/ubuntu/result/import_data.csv", payload=payload, headers=headers)
df = pd.read_csv('/home/ubuntu/result/import_data.csv', sep=",")
